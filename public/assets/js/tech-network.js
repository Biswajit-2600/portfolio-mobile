...
(function () {
  ...
  function initNetwork() {
    ...
    for (let latRing = 0; latRing <= CONFIG.LATITUDE_RINGS; latRing++) {
      ...
      const pointsOnRing = latRing === 0 || latRing === CONFIG.LATITUDE_RINGS 
  ...
  function drawGlobeMesh() {
    ...
    for (let latRing = 0; latRing <= CONFIG.LATITUDE_RINGS; latRing++) {
      ...
      const avgZ = nodesOnRing.reduce((sum, n) => sum + n.z, 0) / nodesOnRing.length;
      const depthFactor = (avgZ + CONFIG.BASE_RADIUS * zoomLevel) / (CONFIG.BASE_RADIUS * zoomLevel * 2);
      const opacity = CONFIG.LINE_OPACITY * Math.max(0.15, Math.min(0.7, depthFactor));
    ...
    for (let lonLine = 0; lonLine < CONFIG.LONGITUDE_LINES; lonLine++) {
      ...
      for (let latRing = 0; latRing <= CONFIG.LATITUDE_RINGS; latRing++) {
        if (latRing === 0 || latRing === CONFIG.LATITUDE_RINGS) {
          ...
          const node = iconNodes.find(n => n.latRing === latRing && n.lonPoint === lonLine);
      ...
      const avgZ = nodesOnMeridian.reduce((sum, n) => sum + n.z, 0) / nodesOnMeridian.length;
      const depthFactor = (avgZ + CONFIG.BASE_RADIUS * zoomLevel) / (CONFIG.BASE_RADIUS * zoomLevel * 2);
      const opacity = CONFIG.LINE_OPACITY * Math.max(0.15, Math.min(0.7, depthFactor));
  ...
  function drawIcons() {
    ...
    iconNodes.forEach(node => {
      ...
      const depthFactor = (node.z + CONFIG.BASE_RADIUS * zoomLevel) / (CONFIG.BASE_RADIUS * zoomLevel * 2);
  ...
  async function loadFonts() {
    ...
    const solidRes = await detect(
      ['Font Awesome 7 Free', 'Font Awesome 6 Free', 'Font Awesome 6 Free Solid', 'Font Awesome 5 Free'],
      ['900', '700']
    );
    if (solidRes) {
